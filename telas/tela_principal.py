...
class TelaPrincipal():
    ...
    def fAtualizarProduto(self):
        try:
            ...
            print('Não foi possível atualizar o produto.', e)

    def atualizarDados(self, codigo, nome, preco):
        try:
            query = f"UPDATE public.produtos SET nome='{nome}', preco='{preco}' WHERE codigo='{codigo}'"
            self.db_pg_context.conectar()
            self.db_pg_context.executar_update_sql(query)
            self.db_pg_context.desconectar()
        except Exception as e:
    ...
    def fExcluirProduto(self):
        try:
            ...
            print('Não foi possível excluir o produto.', e)

    def excluirDados(self, codigo):
        try:
            query = f"DELETE FROM public.produtos WHERE codigo='{codigo}'"
            self.db_pg_context.conectar()
            self.db_pg_context.executar_update_sql(query)
            self.db_pg_context.desconectar()
        except Exception as e:
